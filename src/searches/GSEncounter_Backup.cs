...
public class TestEncounterState {
...
public static class TestEncounter {
    ...
    public static void MakeSave(byte x, byte y, byte hour, byte minute, byte momStep, byte audio, byte frameType, byte menuAccount, byte igt) {
        byte[] baseSave = File.ReadAllBytes("basesaves/gold_sentret_" + x + "_" + y + ".sav");
        ...
        File.WriteAllBytes("roms/gold_sentret_temp.sav", baseSave);
    }
    ...
    */
    ...
    public static void StartSearch(int numThreads) {
        ...
        GscMap r29 = dummyGb.Maps[6147];
        GscMap guardHouse = dummyGb.Maps[6157];
        GscMap r46 = dummyGb.Maps[1289];
        
        Pathfinding.GenerateEdges<GscMap, GscTile>(dummyGb, 0, r29[27, 2], Action.Left | Action.Right | Action.Up | Action.Down);
        Pathfinding.GenerateEdges<GscMap, GscTile>(dummyGb, 0, guardHouse[4, 7], Action.Right | Action.Up);
        Pathfinding.GenerateEdges<GscMap, GscTile>(dummyGb, 0, r46[7, 33], Action.Left | Action.Right | Action.Up | Action.Down);

        r29[27, 2].AddEdge(0, new Edge<GscMap, GscTile> { Action = Action.Up, Cost = 0, NextEdgeset = 0, NextTile = guardHouse[4, 7] });
        
        guardHouse[4, 0].AddEdge(0, new Edge<GscMap, GscTile> { Action = Action.Up, Cost = 0, NextEdgeset = 0, NextTile = r46[7, 33] });
        guardHouse[5, 0].AddEdge(0, new Edge<GscMap, GscTile> { Action = Action.Up, Cost = 0, NextEdgeset = 0, NextTile = r46[8, 33] });

        dummyGb.Dispose();


        GscTile[] startTiles = { r29[59, 8] };
        ...
        Writer = new StreamWriter("gold_sentret_" + DateTime.Now.Ticks + ".txt");
                                    ...
                                    for(byte igt = 0; igt < 60; igt += 6) {
                                        ...
                                        new Thread(parameter => {
                                            ...
                                            lock(startTiles) {
                                                ...
                                                gb = new Gold("roms/gold_sentret_temp.sav");
                                                // gb.Show(); // TODO : test display gb
                                                gb.SetTimeSec(120);
                                                    ...
                                                    for(int delay = 0; delay <= MaxCost; delay++) {
                                                        ...
                                                        DFParameters<Gold, GscMap, GscTile> parameters = new DFParameters<Gold, GscMap, GscTile>() {
                                                            ...
                                                            */
                                                            ...
                                                            {
                                                                // Console.WriteLine("[" + threadIndex + "] " + gb.EnemyMon.Species.Name + "-" + gb.EnemyMon.DVs + "-" + state.tile);
                                                                return gb.EnemyMon.Species.Name == "PIDGEY"
                                                                    && gb.EnemyMon.DVs.Attack >= 10 && gb.EnemyMon.DVs.Defense >= 10 && gb.EnemyMon.DVs.Speed >= 10 && gb.EnemyMon.DVs.Special >= 5;
                                                            },
                                                            //FoundCallback = state => EvaluateTototdile(gb, state),
                                                            SingleCallback = (state,gb) =>
                                                            {
                                                                String line = state.Tile.X + "," + state.Tile.Y + "/" + state.Log + "  " + gb.EnemyMon.Species.Name + " L" + gb.EnemyMon.Level + " DVs: " + gb.EnemyMon.DVs.ToString() + " Cost: " + state.WastedFrames;
                                                                Writer.WriteLine(line);
                                                                Console.WriteLine(line);
                                                                //Trace.WriteLine(tile.X + "," + tile.Y + "/" + state.Log + "  " + gb.EnemyMon.Species.Name + " L" + gb.EnemyMon.Level + " DVs: " + gb.EnemyMon.DVs.ToString() + " Cost: " + state.WastedFrames);
                                                            }
                                                        };
